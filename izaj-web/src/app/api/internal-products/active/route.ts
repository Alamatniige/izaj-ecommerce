...
// Mock product data - same as in the main route
const mockProducts = [
  {
    id: '1',
    product_id: 'P001',
...
export async function GET(request: NextRequest) {
  try {
    ...
    let filteredProducts = mockProducts.filter(p => p.status === status);
    ...
    console.error('Error in active products API route:', error);
    return NextResponse.json(
      { success: false, error: 'Failed to fetch active products' },
      { status: 500 }
    );
  }
}
