...
const ProductListFilterDrawer: React.FC<ProductListFilterDrawerProps> = ({
  ...
  // Local inputs to mirror desktop sidebar behavior
  const [minInput, setMinInput] = useState<string>('');
  const [maxInput, setMaxInput] = useState<string>('');

  // Track initial values when modal opens to know if user changed anything
  const initialPriceRangeRef = useRef<{ min: number; max: number } | null>(null);
  const initialSortRef = useRef<string | null>(null);

  useEffect(() => {
    if (isOpen) {
      initialPriceRangeRef.current = { min: priceRange.min, max: priceRange.max };
  ...
  const hasChanges = (() => {
    const initPrice = initialPriceRangeRef.current;
    const initSort = initialSortRef.current;
    if (!initPrice || initSort == null) return false;
    return (
      initPrice.min !== priceRange.min ||
      initPrice.max !== priceRange.max ||
      initSort !== sortOption
    );
  })();
  ...
  return (
    <>
      ...
      />
      {/* Bottom Sheet Modal with slide up/down */}
      <div
      ...
      >
        {/* Top Bar removed on mobile as requested */}
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-28 pt-6">
            ...
            <div className="mt-4">
              ...
              <div className="mt-5 relative h-4 max-w-full">
                ...
                <div 
                  ...
                  style={{ left: `${minSliderPosition}%`, width: `${maxSliderPosition - minSliderPosition}%` }}
                />
                <input
                  type="range"
                  min="0"
              ...
              {['Alphabetically, A-Z','Alphabetically, Z-A','Price, low to high','Price, high to low','Date, old to new','Date, new to old'].map(option => (
                <div key={option} className="flex items-center cursor-pointer" onClick={() => setSortOption(option)}>
                  <span className={`w-4 h-4 rounded-full border-2 mr-3 ${
                    sortOption === option ? 'bg-black border-black' : 'bg-gray-200 border-gray-300'
                  }`}></span>
          ...
          {/* Categories removed in mobile modal to match desktop sidebar content only */}
          </div>
        </div>
